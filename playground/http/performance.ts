import { startServer, stopServer, SERVER_URL } from './_globals';
...
async function runTests(): Promise<{ fetchSamples: number[]; clientSamples: number[] }> {
    ...
    for (let i = 0; i < RUNS; i++) {
        fetchSamples.push(await measure(() => fetch(SERVER_URL).then((r) => r.json())));
        clientSamples.push(await measure(() => client.get(SERVER_URL, { responseType: 'json' })));
